...
mymain ( int argc, char ** argv )
{
    ...
    if ( matrix == "" )
    {
        ...
        A = Matrix::OMP::build( bct->root(), *pcoeff, *lrapx, acc );
    }// if
    else
    {
        ...
        A = Matrix::OMP::copy( *A ); // for spreading memory usage
    }// else
    ...
    {
        std::cout << term::yellow << term::bold << "∙ " << term::reset << term::bold << "LU ( DAG OMP )" << term::reset << std::endl;
        
        auto  C = A->copy();
        
        tic = Time::Wall::now();

        auto  dag = HLR::DAG::gen_LU_dag( C.get(), HLR::DAG::OMP::refine );
