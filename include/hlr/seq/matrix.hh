        ...
        const size_t                     ntile )
{
    ...
    if ( std::min( D.nrows(), D.ncols() ) <= ntile )
    {
        ...
        {
            return std::make_unique< hpro::TRkMatrix >( rowis, colis, std::move( U ), std::move( V ) );
        }// if
        else
        {
            return std::make_unique< hpro::TDenseMatrix >( rowis, colis, blas::copy( D ) );
        }// else
    }// if
    else
    {
        ...
        for ( uint  i = 0; i < 2; ++i )
        {
            for ( uint  j = 0; j < 2; ++j )
            {
                ...
                sub_D(i,j) = build( sub_rowis[i], sub_colis[j], D_sub, acc, approx, ntile );
        ...
        if ( all_lowrank )
        {
            ...
            for ( uint  i = 0; i < 2; ++i )
                for ( uint  j = 0; j < 2; ++j )
                    rank_sum += ptrcast( sub_D(i,j).get(), hpro::TRkMatrix )->rank();
            ...
            for ( uint  i = 0; i < 2; ++i )
            {
                for ( uint  j = 0; j < 2; ++j )
                {
                    auto  Rij   = ptrcast( sub_D(i,j).get(), hpro::TRkMatrix );
                    auto  Uij   = blas::mat_U< value_t >( Rij );
                    auto  Vij   = blas::mat_V< value_t >( Rij );
            ...
            {
                return std::make_unique< hpro::TRkMatrix >( rowis, colis, std::move( W ), std::move( X ) );
            }// if
        }// if
