...
namespace hlr { namespace hpx { namespace uniform { namespace detail {
...
//
// chpxute mat-vec M·x = y with uniform vectors x,y.
    ...
    if ( cb.nsons() > 0 )
    {
        define_task_block( [&] ( auto &  tb )
        {
            for ( uint  i = 0; i < cb.nsons(); ++i )
                tb.run( [&,i] () { u->set_block( i, scalar_to_uniform( *cb.son(i), v ).release() ); } );
        } );
    }// if
...
make_uniform ( const cluster_basis< value_t > &  cb )
{
    ...
    if ( cb.nsons() > 0 )
    {
        ...
        {
            for ( uint  i = 0; i < cb.nsons(); ++i )
                tb.run( [&,i] () { u->set_block( i, make_uniform( *cb.son(i) ).release() ); } );
        } );
    }// if
    ...
    if ( u.nblocks() > 0 )
    {
        define_task_block( [&] ( auto &  tb )
        {
            for ( uint  i = 0; i < u.nblocks(); ++i )
                tb.run( [&,i] () { add_uniform_to_scalar( *u.block(i), v ); } );
        } );
    }// if
}
...
}}}}// namespace hlr::hpx::uniform::detail
