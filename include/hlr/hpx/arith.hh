             ...
             const TTruncAcc &  acc )
{
    ...
    if ( is_blocked( A ) )
    {
        ...
        ::hpx::parallel::v2::define_task_block(
            ...
            {
                // T_01 = A_00⁻¹ · A_01
                tb.run( [&] () { multiply( 1.0, apply_normal, MA(0,0), apply_normal, MA(0,1), 0.0, MT(0,1), acc ); } );
        
                // T_10 = A_10 · A_00⁻¹
                tb.run( [&] () { multiply( 1.0, apply_normal, MA(1,0), apply_normal, MA(0,0), 0.0, MT(1,0), acc ); } );
            } );
        ...
        ::hpx::parallel::v2::define_task_block(
            ...
            {
                // A_01 = - T_01 · A_11
                tb.run( [&] () { multiply( -1.0, apply_normal, MT(0,1), apply_normal, MA(1,1), 0.0, MA(0,1), acc ); } );
            
                // A_10 = - A_11 · T_10
                tb.run( [&] () { multiply( -1.0, apply_normal, MA(1,1), apply_normal, MT(1,0), 0.0, MA(1,0), acc ); } );
            } );
