#ifndef __HLR_HPX_CLUSTER_BASIS_HH
#define __HLR_HPX_CLUSTER_BASIS_HH
//
// Project     : HLR
                   ...
                   const accuracy &       acc )
{
    ...
    ::hpx::parallel::v2::define_task_block(
        [&] ( auto &  tb )
        {
            tb.run( [&] { detail::build_lrmatrix_map( rowct, M, row_map, false ); } );
            tb.run( [&] { detail::build_lrmatrix_map( colct, M, col_map, true  ); } );
        } );
    ...
    ::hpx::parallel::v2::define_task_block(
        [&] ( auto &  tb )
        {
            tb.run( [&] { row_cb = detail::construct_basis< value_t >( rowct, row_map, acc, false ); } );
            tb.run( [&] { col_cb = detail::construct_basis< value_t >( colct, col_map, acc, true  ); } );
        } );
...
#endif // __HLR_HPX_CLUSTER_BASIS_HH
