            ...
            hosvd_func_t &&                   hosvd )
{
    ...
    for ( uint  l = 0; l < 2; ++l )
    {
        ...
        for ( uint  j = 0; j < 2; ++j )
        {
            ...
            T_jl[j] = std::move( merge( is0, T_0jl->is(1), T_0jl->is(2), D_jl, *T_0jl, *T_1jl, acc, apx, hosvd ) );
        ...
        T_l[l] = std::move( merge( is0, is1, T_jl[0]->is(2), D_l,
    ...
    auto  T_merged = merge( is0, is1, is2, D,
    ...
    for ( uint  l = 0; l < 2; ++l )
    {
        for ( uint  j = 0; j < 2; ++j )
        {
            for ( uint  i = 0; i < 2; ++i )
            {
                ...
                auto  [ U0, S0 ] = blas::svd( GX0, true );  // TODO: only U_i is needed !!!
