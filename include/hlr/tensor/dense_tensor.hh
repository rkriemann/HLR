...
//
bool
is_dense_tensor3 ( const has_value_type auto &  t )
{
...
//
// implements dense (full) 4D tensor
// - storage layout is column-major
//
template < typename T_value >
class dense_tensor4 : public base_tensor4< T_value >
{
public:
    using  value_t = T_value;
    using  real_t  = Hpro::real_type_t< value_t >;
...
public:
    ...
    dense_tensor4 ( const dense_tensor4 &  t )
            : super_t( t )
            , _tensor( t._tensor.size() )
    {
        blas::copy( t._tensor, _tensor );
    ...
    dense_tensor4 ( dense_tensor4 &&  t )
            : super_t( std::forward< base_tensor4< value_t > >( t ) )
    {
        std::swap( _tensor, t._tensor );
    }

    dense_tensor4 ( const indexset &  is0,
                    const indexset &  is1,
                    const indexset &  is2,
                    const indexset &  is3 )
            : super_t( is0, is1, is2, is3 )
            , _tensor( is0.size(), is1.size(), is2.size(), is3.size() )
    {}
    
    dense_tensor4 ( const std::array< indexset, dimension > &  ais )
            : super_t{ ais[0], ais[1], ais[2] }
            , _tensor( ais[0].size(), ais[1].size(), ais[2].size() )
    {}
    ...
    dense_tensor4 ( const indexset &             is0,
                    ...
                    const indexset &             is2,
                    const indexset &             is3,
                    blas::tensor4< value_t > &&  t )
            : super_t( is0, is1, is2, is3 )
            , _tensor( std::move( t ) )
    {}
    
    // dtor
    virtual ~dense_tensor4 ()
    {}

    // assignment
    dense_tensor4 &  operator = ( const dense_tensor4 &  t )
    {
        super_t::operator = ( t );
    ...
    dense_tensor4 &  operator = ( dense_tensor4 &&  t )
    {
        super_t::operator = ( std::forward( t ) );
    ...
    tensor () const
    {
        if ( is_compressed() )
        {
            auto  dT = blas::tensor4< value_t >( this->dim(0),
                                                 this->dim(1),
                                                 this->dim(2),
                                                 this->dim(3) );
    ...
    value_t          coeff       ( const uint  i0,
                                   const uint  i1,
                                   const uint  i2,
                                   const uint  i3 ) const
    {
        HLR_DBG_ASSERT( ! is_compressed() );
        return this->_tensor(i0,i1,i2,i3);
    }
    value_t &        coeff       ( const uint  i0,
                                   const uint  i1,
                                   const uint  i2,
    ...
    value_t          operator () ( const uint  i0,
                                   const uint  i1,
                                   const uint  i2,
                                   const uint  i3 ) const { return coeff( i0, i1, i2, i3 ); }
    value_t &        operator () ( const uint  i0,
                                   const uint  i1,
                                   const uint  i2,
                                   const uint  i3 )       { return coeff( i0, i1, i2, i3 ); }
    ...
    {
        auto  T = super_t::copy();
        auto  X = ptrcast( T.get(), dense_tensor4< value_t > );
    ...
    create () const
    {
        return std::make_unique< dense_tensor4< value_t > >();
    }
    ...
    // return name of type
    virtual std::string  typestr () const { return "dense_tensor4"; }
...
template < typename value_t >
void
dense_tensor4< value_t >::compress ( const Hpro::TTruncAcc &  acc )
{
    if ( is_compressed() )
        return;
...
template < typename value_t >
void
dense_tensor4< value_t >::decompress ()
{
    if ( ! is_compressed() )
        return;
...
//
bool
is_dense_tensor4 ( const has_value_type auto &  t )
{
    using type_t  = std::remove_cv_t< std::remove_reference_t< decltype( t ) > >;
    using value_t = typename type_t::value_t;
    
    return dynamic_cast< const dense_tensor4< value_t > * >( &t ) != nullptr;
}

}}// namespace hlr::tensor

#endif // __HLR_TENSOR_DENSE_TENSOR4_HH
