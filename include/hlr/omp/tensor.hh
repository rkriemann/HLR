#ifndef __HLR_OMP_TENSOR_HH
#define __HLR_OMP_TENSOR_HH
//
// Project     : HLR
// Module      : omp/tensor
// Description : tensor algorithms using OMP
...
namespace hlr { namespace omp { namespace tensor {
        ...
        #pragma omp taskgroup
        {
            ...
            for ( uint  i2 = 0; i2 < 2; ++i2 )
            {
                for ( uint  i1 = 0; i1 < 2; ++i1 )
                {
                    ...
                    {
                            const auto  D_sub = D( sub_is0[i0] - is0.first(),
                                                   sub_is1[i1] - is1.first(),
                                                   sub_is2[i2] - is2.first() );
                            
                            sub_D(i0,i1,i2) = build_hierarchical_tucker( sub_is0[i0], sub_is1[i1], sub_is2[i2], D_sub, ntile, sacc, apx, fpacc );
                            
                            HLR_ASSERT( sub_D(i0,i1,i2).get() != nullptr );
                    }// for
                }// for
            }// for
        }// omp taskgroup
           ...
           blas::tensor3< value_t > &       D )
{
    if ( is_structured_tensor3( X ) )
    {
        ...
        {
            #pragma omp taskloop collapse(3) default(shared)
            for ( uint  i2 = 0; i2 < BX->nblocks(2); ++i2 )
            {
                for ( uint  i1 = 0; i1 < BX->nblocks(1); ++i1 )
                {
                    for ( uint  i0 = 0; i0 < BX->nblocks(0); ++i0 )
                    {
                        if ( ! is_null( BX->block(i0,i1,i2) ) )
                            to_dense( *BX->block(i0,i1,i2), D );
                    }// for
                }// for
            }// for
        }// omp taskgroup
...
}}}// namespace hlr::omp::tensor

#endif // __HLR_OMP_TENSOR_HH
