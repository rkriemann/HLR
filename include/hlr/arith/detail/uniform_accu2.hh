...
// #define ACCU2_TIC       auto  __tic = timer::now()
// #define ACCU2_TOC( t )  { auto  __toc = timer::since( __tic ); t += __toc.seconds(); }
#define ACCU2_TIC       {}
#define ACCU2_TOC( t )  {}
...
extern double  t_eval_lower;
extern double  t_eval_upper;
extern double  t_eval_lu;
extern double  t_eval_uni;
extern double  t_eval_uniA;
extern double  t_eval_uniB;
extern double  t_eval_dense;
...
blas::matrix< value_t >
row_basis ( const uniform_map_t &  rowmap,
            const indexset &       rowis )
{
    if ( rowmap.find( rowis ) == rowmap.end() )
        return blas::matrix< value_t >();
    
    for ( auto  M : rowmap.at( rowis ) )
    {
        if ( matrix::is_uniform_lowrank( M ) )
            return ptrcast( M, matrix::uniform_lrmatrix< value_t > )->row_basis();
    }// for
...
template < typename value_t >
blas::matrix< value_t >
col_basis ( const uniform_map_t &  colmap,
            const indexset &       colis )
{
    if ( colmap.find( colis ) == colmap.end() )
        return blas::matrix< value_t >();
    
    for ( auto  M : colmap.at( colis ) )
    {
        if ( matrix::is_uniform_lowrank( M ) )
            return ptrcast( M, matrix::uniform_lrmatrix< value_t > )->col_basis();
    }// for
...
struct accumulator
{
    ...
    eval ( const value_t                    alpha,
           ...
           const hpro::TTruncAcc &          acc,
           const approx_t &                 approx,
           const blas::matrix< value_t > &  row_basis,
           const blas::matrix< value_t > &  col_basis )
    {
        ...
        if ( ! handle_dense )
        {
            ...
            if ( n_uniB >= 1 )
            {
                ...
                ACCU2_TOC( t_eval_uniB );
            }// if
        }// if

        // //
        // // dense × something or something × dense
        // //
        ...
        if ( ! is_null( BC ) )
        {
            ...
            for ( uint  i = 0; i < BC->nblock_rows(); ++i )
            {
                for ( uint  j = 0; j < BC->nblock_cols(); ++j )
                {
                    sub_accu(i,j).eval( alpha, *BC->block(i,j), acc, approx, row_basis, col_basis );
                  ...
                  const uniform_map_t &    colmap ) //, hpro::TMatrix &          REF )
{
    ...
    accu.eval( value_t(1), M, acc, approx,
               row_basis< value_t >( rowmap, M.row_is() ),
               col_basis< value_t >( colmap, M.col_is() ) );

    ACCU2_TOC( t_eval_lower );
    }
...
// hpro::TMatrix &          REF )
{
    ...
    accu.eval( value_t(1), A, acc, approx,
               row_basis< value_t >( rowmap, A.row_is() ),
               col_basis< value_t >( colmap, A.col_is() ) );               
