...
program_main ()
{
    ...
    if ( cmdline::capprox == "hca" )
    {
        if constexpr ( problem_t::supports_hca )
        {
            ...
            std::tie( rowcb, colcb, A ) = impl::matrix::build_uniform_lvl( bct->root(), pcoeff, hcalr, cbapx, acc );
        }// if
        else
    ...
    if (( cmdline::capprox == "aca" ) || ( cmdline::capprox == "default" ))
    {
        ...
        std::tie( rowcb, colcb, A ) = impl::matrix::build_uniform_lvl( bct->root(), pcoeff, acalr, cbapx, acc );
    }// else
    ...
    if ( cmdline::capprox == "dense" )
    {
        ...
        std::tie( rowcb, colcb, A ) = impl::matrix::build_uniform_lvl( bct->root(), pcoeff, dense, cbapx, acc );
    }// else
