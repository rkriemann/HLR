...
{
    double  h = 4 * math::pi< value_t >() / (n+1);
    auto    M = blas::matrix< value_t >( n, n );
...
program_main ()
{
    ...
    std::cout << "  " << term::bullet << term::bold << "compression, ε = " << delta << ", " << cmdline::approx << term::reset << std::endl;
        
    if ( cmdline::approx == "svd"     || cmdline::approx == "all" ) do_compress< hlr::approx::SVD< value_t > >(     D, delta, norm_D, mem_D );
    if ( cmdline::approx == "rrqr"    || cmdline::approx == "all" ) do_compress< hlr::approx::RRQR< value_t > >(    D, delta, norm_D, mem_D );
    if ( cmdline::approx == "randsvd" || cmdline::approx == "all" ) do_compress< hlr::approx::RandSVD< value_t > >( D, delta, norm_D, mem_D );
    if ( cmdline::approx == "aca"     || cmdline::approx == "all" ) do_compress< hlr::approx::ACA< value_t > >(     D, delta, norm_D, mem_D );
