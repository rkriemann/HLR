...
// Project     : HLib
// File        : hodlr-omp.cc
// Description : HODLR arithmetic using OpenMP
...
mymain ( int argc, char ** argv )
{
    ...
    {
        std::cout << term::yellow << term::bold << "∙ " << term::reset << term::bold << "LU ( HODLR OMP )" << term::reset << std::endl;
        
        auto  C = A->copy();
        
        tic = Time::Wall::now();

        HODLR::OMP::lu< HLIB::real >( C.get(), fixed_rank( k ) );
